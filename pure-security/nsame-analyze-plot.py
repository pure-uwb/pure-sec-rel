...
def nsame_to_probability(nsame):
    if nsame < 2048:
        nsame = 4096 - nsame 
    return binom.sf(nsame, 4096, 0.5) * 2
...
def success_rate_full_plot(data):
    extra = 6
    grouped = data.groupby("nsame")["maxFpHeight"].agg(["mean", "std", "max", "count"])
    security_bits = [np.log2(nsame_to_probability(nsame)) for nsame in grouped.index]
    data["securityBits"] = np.log2(data["nsame"].apply(nsame_to_probability))
    y = np.array(security_bits)+extra
    x = grouped["mean"]+3*grouped["std"]
    filter_points = y <= 0
    y = y[filter_points]
    x = x[filter_points]
    ax = plt.subplot()
    ...
    plt.xlim(250, 1150)
    plt.ylim(-147, 8)
    #plt.scatter(grouped["mean"]+3*grouped["std"], np.array(security_bits)+extra)
    plt.axvline(x=702, color='grey')
    plt.axhline(y=-55+extra, color='grey')
    plt.scatter(702, -55+extra, color='grey')
    plt.text(710, -51+extra, "($T=702$, $SR=2^{-49}$)", fontsize=14)
    # ax.text(0.05, 0.95, textstr, transform=ax.transAxes, fontsize=14,
        # verticalalignment='top', bbox=props)


    plt.grid()
    plt.xlabel("Peak Height Absolute Threshold ($T$)", fontsize = 14)
    plt.ylabel("Log2(Attack Success Rate)", fontsize = 14)
    ax.set_box_aspect(0.7)
    plt.legend()
    plt.savefig("./diagrams/sr_vs_threshold_zoom_strong.pdf", bbox_inches='tight')
    plt.show()
...
def success_rate_importance_sampling(data):
    ...
    plt.scatter(grouped["mean"]+3*grouped["std"],
            security_bits)
    plt.axvline(x=702, color='grey')
    plt.axhline(y=-55, color='grey')
    plt.scatter(702, -55, color=-'grey')

    #plt.scatter(grouped["mean"], security_bits,
    #        s=4, label="underestimate")

    #plt.plot(thresholds, np.log2(srs), '+' , linewidth=0.1, label="log_2(p_a)")
    #plt.plot(thresholds, np.log2(srs+3*sems), '+', linewidth=0.1, label="$log_2(p_a+3.291\sigma)$")
    #plt.plot(thresholds, np.log2(srs-3*sems), '+', linewidth=0.1, label="$log_2(p_a-3.291\sigma)$")
 
    #plt.axhline(y=-51)

    # plt.grid()
    # plt.xlabel("Peak Height Absolute Threshold ($T$)")
    # plt.ylabel("Attack Success Probability ($log2(p_{a})$)")
